...
class GetSurveySummaryUseCase
{
    ...
    public function execute(int $surveyId, Request $request): array
    {
        ...
        foreach ($orderedNames as $name) {
            ...
            foreach ($responses as $resp) {
                $snapshot = $resp['survey_snapshot_json'] ?? [];
                $elementsInSnapshot = $this->flattenElementsCached($snapshot);
                $elInSnapshot = null;
    ...
    private function aggregate(array &$data, string $category, array $el, $value, bool $existsInSnapshot): void
    {
        ...
        switch ($category) {
            ...
            case 'boolean':
                if ($existsInSnapshot) {
                    $key = ($value === null) ? ($el['valueFalse'] ?? 0) : $value;
